...
pub struct LabelFilters(pub(crate) UnitVec<UnitVec<Matcher>>);
...
impl LabelFilters {
    pub fn new(name: String) -> Self {
        ...
        let filters = unitvec![name_matcher];
        LabelFilters(unitvec![filters])
    }
    ...
    pub fn append_matcher(&mut self, matcher: Matcher) {
        if let Some(last_matchers) = self.0.last_mut() {
            if matcher.is_metric_name_filter() {
                last_matchers.insert(0, matcher);
            } else {
                last_matchers.push(matcher);
            }
        } else {
            let new_matchers = unitvec!(matcher);
            self.0.push(new_matchers);
        }
    }

    pub fn append_or(&mut self, matcher: Matcher) {
        let new_matchers = unitvec!(matcher);
        self.0.push(new_matchers);
    }

    pub fn append(&mut self, matchers: Vec<Matcher>) {
        self.0.push(matchers.into());
    }
    ...
    pub fn find_matchers(&self, name: &str) -> Vec<&Matcher> {
        self.0
            .iter()
            .flat_map(|matchers| matchers.iter().filter(|m| m.label.eq(name)))
            .collect()
    }

    pub fn sort(&mut self) {
        for matchers in self.0.iter_mut() {
            matchers.sort();
        }
    }
    ...
    pub fn get_metric_name(&self) -> Option<&str> {
        ...
        if let Some(matchers) = self.0.first() {
            if matchers.is_empty() {
                return None;
            }
            let metric_name = matchers.iter().find(|m| m.is_metric_name_filter());
            if let Some(metric_name) = metric_name {
                for matchers in self.0.iter().skip(1) {
                    if let Some(metric) = matchers.iter().find(|m| m.is_metric_name_filter()) {
                        if metric.value != metric_name.value {
                            return None;
                        }
                    }
                }
    ...
    pub fn normalize(&mut self) {
        ...
        let first = self.0.first().unwrap(); // safe to unwrap since we checked for empty
        ...
        for match_list in self.0.iter().skip(1) {
            let mut found = false;
        ...
        let mut saved_matcher = Matcher::default();
        let mut got_matcher = false;
        ...
        for (i, j, remove) in to_remove.iter().rev() {
            if *remove {
                ...
                if !got_matcher {
                    got_matcher = true;
                    saved_matcher = self.0[*i].remove(*j);
                } else {
                    self.0[*i].remove(*j);
                }
            }
        ...
        // reinsert the metric name filter at the front
        if got_matcher {
            self.0[0].insert(0, saved_matcher);
        }
    }
}

impl Display for LabelFilters {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        if self.is_empty() {
            write!(f, "{{}}")?;
            return Ok(());
        ...
        // if we have more than one matcher, we need to print the braces
        let is_name_only = self.0.iter().all(|m| m.len() == 1);
        ...
        for (i, matchers) in self.0.iter().enumerate() {
            let matchers = if i == 0 && offset > 0 {
                &matchers[offset..]
            } else {
                &matchers[0..]
            };
